...
class SlabABgC2d(object):
    ...
    def build_grid(self):
        ...
        if os.path.exists(config.grid.field_data):
            ...
            self.wA = np.random.rand(Lx, Ly)
            self.wB = np.random.rand(Lx, Ly)
            self.wC = np.random.rand(Lx, Ly)
        ...
        self.qBc = np.zeros((MsB, Lx, Ly))
        self.qBc[0, :, :] = 1.
        self.qC = np.zeros((MsC, Lx, Ly))
        self.qC[0, :, :] = 1.
        self.qCc = np.zeros((MsC, Lx, Ly))
        ...
        self.qBc_solver = ETDRK4FxCy(La, Lb, Lx, Ly-1,
                                     MsB, h=ds, lbc=lbcB, rbc=rbcB)
        self.qC_solver = ETDRK4FxCy(La, Lb, Lx, Ly-1,
                                    MsC, h=ds, lbc=lbcC, rbc=rbcC)
        self.qCc_solver = ETDRK4FxCy(La, Lb, Lx, Ly-1,
                                     MsC, h=ds, lbc=lbcC, rbc=rbcC)
